...
import { Activity, AlertTriangle, Bot, Check, CheckCircle2, ChevronRight, Clock3, Cpu, Loader2, ShieldCheck, X, Zap, Circle, MessageSquare } from 'lucide-react';
...
type Trace = {
  ...
  hinglishTranscript?: {
    ...
    transitions?: Array<{ currentState: string; customerResponse: string; nextState: string; outcome: string }>;
  ...
  aiDecision: {
    ...
    decisionFactors: Array<{ factor: string; signal: string; value: number; evidence: string }>;
    guardrailChecks: Array<{ name: string; status: string; value: string; reason: string }>;
    // AI source metadata
    source?: string;
    aiProvider?: string;
    aiModel?: string;
  ...
  return (
    <section className="rounded-2xl border border-blue-400/30 bg-[#0b1527] p-4 sm:p-5">
      <div className="mb-4 flex items-center gap-3">
        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-blue-400/30 bg-blue-400/10">
          {isAI ? <Bot className="h-5 w-5 text-blue-300" /> : <Cpu className="h-5 w-5 text-slate-400" />}
        </div>
        <div>
          <p className="text-[11px] font-mono uppercase tracking-[.18em] text-blue-300">AI Decision Evidence</p>
          <h3 className="mt-0.5 text-base font-bold text-white">
            {isAI ? 'AI-Assisted Diagnosis' : 'Deterministic Fallback'}
          </h3>
        </div>
        <span className={`ml-auto rounded-full border px-3 py-1 text-[11px] font-bold tracking-wider ${isAI ? 'border-blue-400/40 bg-blue-400/10 text-blue-200' : 'border-slate-600 bg-slate-800 text-slate-400'}`}>
          {isAI ? '● AI-ASSISTED' : isFallback ? '● DETERMINISTIC FALLBACK' : '● DETERMINISTIC'}
        </span>
      </div>

      {isFallback && (
        <div className="mb-4 rounded-xl border border-amber-400/30 bg-amber-400/5 p-3">
          <p className="text-xs font-semibold text-amber-300">Decision source: Deterministic fallback</p>
          {aiDecision.aiFallbackReason && (
            <p className="mt-1 text-xs text-slate-400">Reason: {aiDecision.aiFallbackReason}</p>
          )}
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">{isAI ? 'Model confidence' : 'Rule confidence'}</p>
          <p className="mt-1 text-sm font-semibold text-white">
            {aiDecision.confidencePercent}% · {aiDecision.confidence}
          </p>
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Root Cause</p>
          <p className="mt-1 text-sm text-slate-200">{aiDecision.aiRootCause ?? aiDecision.detectedIssue}</p>
        </div>
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">AI recommended playbook</p>
          <p className="mt-1 text-sm font-bold text-purple-200">
            {pretty(aiDecision.selectedPlaybook ?? aiDecision.selectedAction)}
          </p>
        </div>
      </div>

      {isAI && aiDecision.aiReasoning && (
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Model Reasoning</p>
          <p className="mt-1 text-sm leading-relaxed text-slate-300">{aiDecision.aiReasoning}</p>
        </div>
      )}

      {isAI && aiDecision.aiRelevantSignals && aiDecision.aiRelevantSignals.length > 0 && (
        <div className="mt-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-500">Key Case Signals</p>
          <div className="flex flex-wrap gap-2">
            {aiDecision.aiRelevantSignals.map((signal, i) => (
              <span key={i} className="rounded-full border border-blue-400/25 bg-blue-400/10 px-3 py-1 text-xs text-blue-200">
                {signal}
              </span>
            ))}
          </div>
      ...
      {isAI && aiDecision.timestamp && (
        <p className="mt-4 text-[11px] text-slate-500">
          Model called at {new Date(aiDecision.timestamp).toLocaleString('en-IN')}
        </p>
      )}

      {/* AI/Guardrail boundary disclosure */}
      <div className="mt-4 rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-3">
        <p className="text-xs font-bold text-cyan-300">AI vs Guardrails — Boundary</p>
        <p className="mt-1 text-xs leading-relaxed text-slate-400">
          AI handles case diagnosis and recovery-playbook recommendation. Execution remains bounded by deterministic guardrails. Retry limits, cooldowns, financial thresholds, duplicate-action protection, settlement verification, and stopping/escalation rules are enforced without relying on model output.
        </p>
      </div>
