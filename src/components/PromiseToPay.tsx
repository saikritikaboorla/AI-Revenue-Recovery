...
import { Target, RefreshCw, CheckCircle2, AlertTriangle, Clock, Calendar, MessageSquare } from 'lucide-react';
    ...
    <div className="rounded-xl border border-[#252D3A] bg-[#141A24] p-6 shadow-xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#252D3A] pb-5">
        <div>
          ...
          <p className="text-xs text-[#98A2B3] mt-1 ml-9">
            B2B structured payment commitments negotiated via AI agent — track fulfilment and broken promise escalation.
          </p>
        </div>

        <button
          onClick={fetchPromises}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-[#252D3A] bg-[#10151F] text-xs font-semibold text-[#98A2B3] hover:text-[#F5F7FA] hover:border-slate-600 transition-all"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl border border-rose-500/20 bg-rose-950/10 p-4">
          ...
          <p className="text-xs text-rose-400/70 mt-0.5">Requires re-escalation</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-8 flex flex-col items-center justify-center gap-3 text-rose-400 text-xs text-center">
          ...
          <button
            onClick={fetchPromises}
            className="px-4 py-2 rounded-lg border border-rose-500/40 bg-rose-950/30 text-rose-300 font-semibold hover:bg-rose-900/40 transition-all"
          >
            Retry
          </button>
        </div>
        ...
        <div className="py-10 text-center text-xs text-[#98A2B3] space-y-2">
          ...
          <p className="text-[#252D3A] text-[10px]">
            Run the Batch Simulator with B2B Overdue Receivables to generate P2P workflows.
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-xs text-[#98A2B3]">
            <thead className="border-b border-[#252D3A] bg-[#080B12] text-[11px] uppercase font-semibold">
              <tr>
                ...
                {onSelectCase && <th className="py-3 px-4 text-right">Trace</th>}
              </tr>
            </thead>
              ...
              {promises.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <span className={`font-mono text-[11px] ${isPast && p.status !== 'KEPT' ? 'text-rose-400' : 'text-slate-300'}`}>
                        {promiseDate
                    ...
                    {onSelectCase && (
                      <td className="py-3 px-4 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg border border-[#252D3A] bg-[#141A24] text-indigo-400 hover:text-indigo-300 hover:border-indigo-500/40 text-[11px] font-semibold transition-all"
                        >
                          View →
                        </button>
                      </td>
